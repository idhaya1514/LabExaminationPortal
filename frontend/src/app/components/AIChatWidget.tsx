  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "model",
      content:
        "Hello! I am your LeetCode Teaching Assistant. What problem are you working on? (I will only provide hints, no code!)",
    },
  ]);
...
If the student asks for the exact code, politely refuse and offer another hint or explain the logic instead.`;
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [
          { role: "user", parts: [{ text: systemInstruction }] },
          {
            ...
            parts: [
              {
                text: "Understood. I will act as a teaching assistant and only provide hints, never code.",
              },
            ],
          },
          ...messages.map((m) => ({
      ...
      if (
        ...
        displayMessage =
          "⚠️ The AI Assistant's daily free usage quota has been reached. Please try again later, or ask your lab coordinator to check the Gemini API billing configuration.";
      } else if (errorStr.includes("key is not configured")) {
        displayMessage =
          "⚠️ Gemini API key is not configured in the system. Please ask your coordinator to set VITE_GEMINI_API_KEY.";
      } else {
