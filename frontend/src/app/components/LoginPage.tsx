...
type TabType = "student" | "register" | "admin" | "staff";
...
export default function LoginPage({
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!/^[A-Z]/.test(name.trim()))
      return setError("Name must start with an uppercase letter.");
    if (!email.trim()) return setError("Please enter your email address.");
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim()))
      return setError("Please enter a valid email address with an '@' symbol.");
    if (!department || !DEPARTMENTS.includes(department))
      return setError("Please select a valid department from the list.");
    if (!registerNumber.trim())
      return setError("Please enter your register number.");
    const regNoPattern = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8}$/;
    if (!regNoPattern.test(registerNumber.trim()))
      return setError(
        "Register number must be exactly 8 alphanumeric characters (e.g., E23AI011).",
      );
    if (!leetCodeUsername.trim())
      return setError("Please enter your LeetCode username.");
    if (
      !leetCodeUrl.trim() ||
      !leetCodeUrl.trim().startsWith("https://leetcode.com/u/")
    )
      return setError(
        "Please enter a valid LeetCode profile URL (e.g., https://leetcode.com/u/username/).",
      );
  ...
  const handleAdminSubmit = async (e: React.FormEvent) => {
    ...
    if (
      adminUsername.trim() === "sscet" &&
      adminPassword === "adminsscet@2026"
    ) {
      onAdminLogin();
    } else {
  ...
  const handleStaffSubmit = async (e: React.FormEvent) => {
    ...
    if (!/^\d{4}$/.test(staffId.trim()))
      return setError("Staff ID must be exactly 4 digits.");
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    if (!forgotEmail.trim())
      return setForgotError("Please enter your registered email.");
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 font-sans">
      ...
      >
        ...
        <div className="flex items-center gap-3 relative z-10">
          ...
          <span className="text-sm font-black tracking-widest text-indigo-100 uppercase">
            SSCET Platform
          </span>
        </div>
        ...
        <div className="my-auto py-12 md:py-0 relative z-10 max-w-lg">
          ...
          <p className="text-sm md:text-base text-indigo-100 leading-relaxed opacity-90 mb-8">
            Welcome to the automated laboratory evaluation portal. Monitor code
            test evaluations, multiple choice questionnaires, and daily LeetCode
            activity directly from a unified panel.
          </p>
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-xs font-semibold text-indigo-100/90">
                Real-time compilation and automatic testcase checking.
              </p>
            </div>
            <div className="flex items-start gap-3">
              <div className="w-5 h-5 rounded-full bg-emerald-500/20 border border-emerald-500/35 flex items-center justify-center mt-0.5">
              ...
              <p className="text-xs font-semibold text-indigo-100/90">
                Live Syncing with student's LeetCode profile solved counts.
              </p>
            </div>
            <div className="flex items-start gap-3">
              <div className="w-5 h-5 rounded-full bg-emerald-500/20 border border-emerald-500/35 flex items-center justify-center mt-0.5">
              ...
              <p className="text-xs font-semibold text-indigo-100/90">
                Pulsing reminders and active dashboard streak logging.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-md mx-auto space-y-6">
          <div className="mb-2">
            ...
            <p
              ...
              style={{ color: "#475569" }}
            >
              Please select your portal type and provide credentials.
            </p>
          </div>

          {/* Tab Switcher (Four Pill Indicator switcher) */}
          <div
            className="relative flex p-1.5 rounded-2xl"
            style={{
          ...
          >
            ...
            <button
              ...
              className={`relative z-10 flex-1 py-2 text-[10px] font-black uppercase tracking-wider rounded-xl transition-colors duration-300 ${
                activeTab === "student" ? "text-indigo-600" : "text-slate-500"
              }`}
            >
              Sign In
            </button>
            <button
              ...
              className={`relative z-10 flex-1 py-2 text-[10px] font-black uppercase tracking-wider rounded-xl transition-colors duration-300 ${
                activeTab === "register" ? "text-indigo-600" : "text-slate-500"
              }`}
            >
              Register
            </button>
            ...
            <button
              ...
              className={`relative z-10 flex-1 py-2 text-[10px] font-black uppercase tracking-wider rounded-xl transition-colors duration-300 ${activeTab === "staff" ? "text-indigo-600" : "text-slate-500"}`}
            >
              Staff
            </button>
          </div>
          ...
          {activeTab === "staff" && (
            <form
              ...
              className="space-y-4 animate-fade-in"
            >
              <Field label="Staff ID" icon={User}>
                <input
                  type="text"
                  value={staffId}
      ...
      {isForgotOpen && (
        ...
        >
          <div
            className="rounded-3xl shadow-2xl w-full max-w-md p-6 relative animate-scale-in"
            style={{
          ...
          >
            ...
            {resetStep === "code" && (
              <form onSubmit={handleVerifyCode} className="space-y-4">
                <div
                  className="rounded-2xl p-4 mb-2 text-xs font-medium"
                  style={{
                ...
                >
                  A 6-digit OTP verification code has been dispatched to your
                  email inbox. Please retrieve the code and submit below.
                </div>
