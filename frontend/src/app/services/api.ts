...
export async function getStudents(): Promise<Student[]> {
  ...
  if (isSupabaseConfigured) {
    try {
      ...
      const mapped = (data ?? [])
        .filter(
          (r) =>
            r.email &&
            r.email.trim() !== "" &&
            r.leet_code_username &&
            r.leet_code_username.trim() !== "",
        )
        .map((r) => ({
          id: r.id,
          name: r.name,
      ...
      // Merge remote students with localStorage students to recover any fallback saves
      const merged = [...mapped];
      for (const l of local) {
        if (
          !merged.some(
            (r) =>
              r.registerNumber.toLowerCase() === l.registerNumber.toLowerCase(),
          )
        ) {
          merged.push(l);
        }
      ...
      // Filter out duplicate names (keeping the first occurrence)
      const uniqueByName = [];
  ...
  try {
    ...
    try {
      const upRes = await fetch(
        `${BASE}/userProfile/${clean}?t=${Date.now()}`,
        fetchOpts,
      );
      if (upRes.ok) {
    ...
    localStorage.setItem(
      cacheKey,
      JSON.stringify({ data: statsResult, timestamp: Date.now() }),
    );
    localStorage.setItem(
      cacheKey,
      JSON.stringify({ data: statsResult, timestamp: Date.now() }),
    ...
    try {
      const fallbackRes = await fetch(
        `https://leetcode-api-faisalshohag.vercel.app/${clean}?t=${Date.now()}`,
        { cache: "no-store" as RequestCache },
      );
      ...
      throw new Error(
        `LeetCode username "${username}" not found or both APIs are down. Please check username.`,
      );
    }
  }
}
